...
- **silent fallback 禁止**。確定できない値を「それっぽい既定値」で埋めて先に進めない。設定・外部API応答・DB値・権限・所有者・状態が確定できない場合は、明示的なエラー、未設定状態、確認導線のいずれかで扱う
...
- 外部API応答、設定、DB値、権限、所有者、状態が確定できない場合は、近い値・先頭値・古い値・一般的な既定値へ黙って置換しない
- 初期値を使う場合は、仕様・設定・DB・ユーザー操作などの一次ソースを明確にし、未設定時だけに限定する
- 保存済み値が不正・不整合な場合は、初期値へ戻さず、エラーまたは管理者が修正できる状態として扱う
- UI 表示用の placeholder と、保存・実行に使う実値を混同しない
