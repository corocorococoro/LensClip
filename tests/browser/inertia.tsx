...
export const calls: { posts: Array<{ url: string; data: FormData; options: any }>; visits: string[]; reloads: number } = {
    posts: [], visits: [], reloads: 0,
};
const listeners = new Map<string, Set<(event: any) => void>>();
let currentPath = '/observations/upload-pending';
export const router = {
    get: (url: string) => router.visit(url),
    reload: (options?: any) => { calls.reloads++; window.dispatchEvent(new Event('test:reload')); options?.onSuccess?.(); },
    visit: (url: string) => {
